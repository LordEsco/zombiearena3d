...
class Room {
    ...
    handlePlayerInput(socketId, data) {
        if (!this.players[socketId] || this.players[socketId].isDead)
            return;
    ...
    handleZombieHit(socketId, zombieId, damage) {
...
export class RoomManager {
    ...
    handleConnection(socket) {
        ...
        socket.on("player_input", (data) => {
            const roomId = this.getRoomId(socket);
            if (roomId)
                this.rooms[roomId].handlePlayerInput(socket.id, data);
        });
        socket.on("zombie_hit", (data) => {
            const roomId = this.getRoomId(socket);
            if (roomId)
                this.rooms[roomId].handleZombieHit(socket.id, data.zombieId, data.damage);
        });
